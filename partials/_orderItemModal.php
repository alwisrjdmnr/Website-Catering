                            ...
                            <tbody>
                                <?php
                                    ...
                                    while($myrow = mysqli_fetch_assoc($myresult)){
                                        $produkId = $myrow['produkId'];
                                        $itemQuantity = $myrow['itemQuantity'];
                                        
                                        $itemsql = "SELECT * FROM `produk` WHERE produkId = $produkId";
                                        $itemresult = mysqli_query($conn, $itemsql);
                                        $itemrow = mysqli_fetch_assoc($itemresult);
                                        $produkName = $itemrow['produkName'];
                                        $produkPrice = $itemrow['produkPrice'];
                                        $produkDesc = $itemrow['produkDesc'];
                                        $produkCategorieId = $itemrow['produkCategorieId'];

                                        echo '<tr>
                                                <th scope="row">
                                                    <div class="p-2">
                                                    <img src="img/produk-'.$produkId. '.jpg" alt="" width="70" class="img-fluid rounded shadow-sm">
                                                    <div class="ml-3 d-inline-block align-middle">
                                                        <h5 class="mb-0"> <a href="#" class="text-dark d-inline-block align-middle">'.$produkName. '</a></h5><span class="text-muted font-weight-normal font-italic d-block">Rp. ' .$produkPrice. '/-</span>
                                                    </div>
                                                    </div>
                                                </th>
